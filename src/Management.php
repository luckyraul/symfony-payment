...
class Management
{
    ...
    public function __construct(
        private Service\Transaction $transaction,
    ...
    {
        return $this->transaction->getSummary($code, $orderId);
    }

    public function isPaid(string $code, string $orderId): bool
    {
        $summary = $this->transaction->getSummary($code, $orderId);
        if ($summary->auth > 0) {
            return true;
        }
    ...
    public function refund(string $code, string $orderId): void
    {
        ...
        $transaction = $this->transaction->findCaptureTransaction($code, $paymentIdentifier);
        if (!$transaction) {
    ...
    public function void(string $code, string $paymentIdentifier): void
    {
        ...
        $transaction = $this->transaction->findAuthTransaction($code, $paymentIdentifier);
        if (!$transaction) {
    ...
    {
        $transaction = $this->transaction->findCaptureTransaction($code, $paymentId);
